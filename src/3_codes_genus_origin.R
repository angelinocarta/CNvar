...
diver3<-read.csv("data/genera.stem.ageORIGIN.csv", header=T)
merged<-merge(merged, diver3, by="genus")

tree<-read.tree("data/origin_angio_2019_unique_GENERA.tree")
...
pdf("results/3.pgls.genera.origin.pdf", width=4, height=8)
par(mfrow=c(5,1))
par(mar = c(0,4,0,4), oma = c(5, 2, 3, 2))
plot( jitter(cdat[[2]]$SD2, factor=0,amount=0.05), (cdat[[2]]$div.rate0), pch = 21, bg="gray", col="darkgray", cex = 1.1 * sqrt(cdat[[2]]$sampling.effort), xlab = "", ylab = "", xlim=c(1,250), ylim=c(0.001, 100), yaxt="n", xaxt="n", log="xy")
legend(45, 120, legend=c("all angiosperms", "monocots", "rosids", "asterids"),col=c("blue", "olivedrab3", "purple", "orange"), lty=c(1,2,2,2), cex=0.6, box.lty=0)
at.y <- outer(1:9, 10^(-3:100))
...
plot( jitter(cdat[[2]]$SD2, factor=0,amount=0.05), (cdat[[2]]$div.rate0.9), pch = 21, bg="gray", col="darkgray", cex = 1.1 * sqrt(cdat[[2]]$sampling.effort), xlab = "", ylab = "", xlim=c(1,250), ylim=c(0.0001, 50), yaxt="n", xaxt="n", log="xy")
at.y <- outer(1:9, 10^(-4:10))
lab.y <- ifelse(log10(at.y) %% 1 == 0, at.y, NA)
axis(2, at=at.y, labels=lab.y, las=1, cex.axis=0.75)
...
plot( jitter(cdat[[2]]$SD2, factor=0,amount=0.05), (cdat[[2]]$r.mean), pch = 21, bg="gray", col="darkgray", cex = 1.1 * sqrt(cdat[[2]]$sampling.effort), xlab = "", ylab = "", xlim=c(1,250), ylim=c(0.01, 5), yaxt="n", xaxt="n", log="xy")
at.y <- outer(1:9, 10^(-2:10))
lab.y <- ifelse(log10(at.y) %% 1 == 0, at.y, NA)
axis(2, at=at.y, labels=lab.y, las=1, cex.axis=0.75)
...
plot( jitter(cdat[[2]]$SD2, factor=0,amount=0.05), (cdat[[2]]$SR), pch = 21, bg="gray", col="darkgray", cex = 1.1 * sqrt(cdat[[2]]$sampling.effort), xlab = "", ylab = "", xlim=c(1,250), ylim=c(1, 2500), yaxt="n", xaxt="n", log="xy")
...
plot( jitter(cdat[[2]]$SD2, factor=0,amount=0.05), (cdat[[2]]$age), pch = 21, bg="gray", col="darkgray", cex = 1.1 * sqrt(cdat[[2]]$sampling.effort), xlab = "", ylab = "", xlim=c(1,250), ylim=c(1, 200), yaxt="n", xaxt="n", log="xy")
...
write.table(tab.res, "results/3.results.tab.genera.origin.csv", row.names=FALSE, sep=",")