...
t<-drop.tip(ttt, as.character(data$family))
...
###
pdf("results/1.pgls.familySB.pdf", width=4, height=8)
par(mfrow=c(5,1))
...
legend(1, 1.2, legend=c("all angiosperms", "monocots", "rosids", "asterids"),col=c("blue", "olivedrab3", "purple", "orange"), lty=c(1,2,2,2), cex=0.6, box.lty=0)
...
plot( jitter(cdat[[2]]$SD2, factor=0,amount=0.05), (cdat[[2]]$r.mean), pch = 21, bg="gray", col="darkgray", cex = 1.1 * sqrt(cdat[[2]]$sampling.effort), xlab = "", ylab = "", xlim=c(1,250), ylim=c(0.01, 1), yaxt="n", xaxt="n", log="xy")
at.y <- outer(1:9, 10^(-3:1))
lab.y <- ifelse(log10(at.y) %% 1 == 0, at.y, NA)
axis(2, at=at.y, labels=lab.y, las=1, cex.axis=0.75)
...
plot( jitter(cdat[[2]]$SD2, factor=0,amount=0.05), (cdat[[2]]$SR), pch = 21, bg="gray", col="darkgray", cex = 1.1 * sqrt(cdat[[2]]$sampling.effort), xlab = "", ylab = "", xlim=c(1,250), ylim=c(1, 30000), yaxt="n", xaxt="n", log="xy")
...
names(tab.res)<-c("clade", "n (sampled families)", "predictor", "intercept", "slope", "P", "Rsq", "lambda")
write.table(tab.res, "results/1.results.tab.familySB.csv", row.names=FALSE, sep=",")
