...
diver3<-read.csv("data/genera.stem.ageMOLINA.csv", header=T)
merged<-merge(merged, diver3, by="genus")

tree<-read.tree("data/genus.tree.MOLINA.tree")
...
data<-subset(data,sampling.effort>0.2)
data$age<-data$stem.age
...
pdf("results/4.pgls.generaMOLINA.pdf", width=4, height=8)
par(mfrow=c(5,1))
par(mar = c(0,4,0,4), oma = c(5, 2, 3, 2))
plot( jitter(cdat[[2]]$SD2, factor=0,amount=0.05), (cdat[[2]]$div.rate0), pch = 21, bg="gray", col="darkgray", cex = 1.1 * sqrt(cdat[[2]]$sampling.effort), xlab = "", ylab = "", xlim=c(1,250), ylim=c(0.001, 10), yaxt="n", xaxt="n", log="xy")
...
plot( jitter(cdat[[2]]$SD2, factor=0,amount=0.05), (cdat[[2]]$div.rate0.9), pch = 21, bg="gray", col="darkgray", cex = 1.1 * sqrt(cdat[[2]]$sampling.effort), xlab = "", ylab = "", xlim=c(1,250), ylim=c(0.0001, 10), yaxt="n", xaxt="n", log="xy")
...
plot( jitter(cdat[[2]]$SD2, factor=0,amount=0.05), (cdat[[2]]$SR), pch = 21, bg="gray", col="darkgray", cex = 1.1 * sqrt(cdat[[2]]$sampling.effort), xlab = "", ylab = "", xlim=c(1,250), ylim=c(0.5, 2000), yaxt="n", xaxt="n", log="xy")
...
plot( jitter(cdat[[2]]$SD2, factor=0,amount=0.05), (cdat[[2]]$age), pch = 21, bg="gray", col="darkgray", cex = 1.1 * sqrt(cdat[[2]]$sampling.effort), xlab = "", ylab = "", xlim=c(1,250), ylim=c(0.5, 200), yaxt="n", xaxt="n", log="xy")
...
write.table(tab.res, "results/4.results.tab.generaMOLINA.csv", row.names=FALSE, sep=",")
